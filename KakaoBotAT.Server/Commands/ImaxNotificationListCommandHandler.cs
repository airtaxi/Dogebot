        ...
        try
        {
            if (!await adminService.IsAdminAsync(data.SenderHash))
            {
                return new ServerResponse
                {
                    Action = "send_text",
                    RoomId = data.RoomId,
                    Message = "⛔ 권한이 없습니다. 관리자만 용아맥 알림을 조회할 수 있습니다."
                };
            }
            ...
            return new ServerResponse
            {
                ...
                Message = $"🔔 용아맥 알림 정보\n\n" +
                         ...
                         $"!용아맥해제로 알림을 해제할 수 있습니다."
            };
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "[IMAX_LIST] Error processing IMAX notification list command");
            return new ServerResponse
            {
                Action = "send_text",
                RoomId = data.RoomId,
                Message = "용아맥 알림 조회 중 오류가 발생했습니다."
            };
        }
    }
}
