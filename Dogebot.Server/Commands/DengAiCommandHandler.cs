...
public class DengAiCommandHandler(IDengAiService dengAiService, IAdminService adminService, IDengAiLongReplyService dengAiLongReplyService, ILogger<DengAiCommandHandler> logger) : ICommandHandler
{
    ...
    public async Task<ServerResponse> HandleAsync(KakaoMessageData data)
    {
        try
        {
            ...
            reply = await ConvertLongReplyToLinkAsync(reply, data.RoomId);
    ...
    private async Task<string> ConvertLongReplyToLinkAsync(string reply, string roomId)
    {
        if (reply.Length < LongReplyMinimumCharacterCount) return reply;
        if (!await dengAiLongReplyService.IsEnabledAsync(roomId)) return reply;
