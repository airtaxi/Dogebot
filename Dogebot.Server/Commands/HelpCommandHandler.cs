...
public class HelpCommandHandler(ILogger<HelpCommandHandler> logger) : ICommandHandler
{
    ...
    [
        new("🎮 게임 & 랜덤", ["게임", "랜덤"], [new("!홀 / !짝", ["!홀", "!짝"], "홀짝 게임"), new("!주사위 (범위)", ["!주사위"], "1~범위 사이 랜덤 숫자 (최대: 18,446,744,073,709,551,615)"), new("확률", ["확률"], "0~100% 랜덤 확률"), new("!선택 (항목1) (항목2) ...", ["!선택"], "랜덤 선택"), new("!뭐먹지", ["!뭐먹지"], "음식 추천"), new("!코스요리", ["!코스요리"], "랜덤 코스요리 추천"), new("!차뽑기", ["!차뽑기"], "랜덤 차량 뽑기"), new("!행성뽑기", ["!행성뽑기"], "노 맨즈 스카이 랜덤 행성 생성"), new("!로또 [회차 수]", ["!로또"], "로또 번호 생성 (최대 10회)"), new("!운세", ["!운세"], "오늘의 운세 (재물운, 성공운, 애정운)")]),
        new("🎭 재미", ["재미"], [new("판사님 (질문)", ["판사님"], "판결 내리기"), new("소라고동님 (질문)", ["소라고동님"], "마법의 소라고동님 소환"), new("댕", ["댕"], "멍멍 왈왈 으르르 컹컹"), new("!햄최몇", ["!햄최몇"], "한번에 먹을 수 있는 햄버거 개수"), new("!퇴근", ["!퇴근"], "재치있는 퇴근 시간 안내")]),
        new("🤖 AI", ["AI", "댕댕아"], [new("댕댕아 (메시지)", ["댕댕아"], "AI 댕댕이 답변"), new("!댕댕링크활성화", ["!댕댕링크활성화"], "이 방의 긴 AI 답변 링크 표시 활성화"), new("!댕댕링크비활성화", ["!댕댕링크비활성화"], "이 방의 긴 AI 답변 링크 표시 비활성화")]),
        new("💬 심심이", ["심심이"], [new("심심아 (메시지)", ["심심아"], "등록된 답변 조회"), new("!심등록 (메시지) / (답변)", ["!심등록"], "답변 등록 (개인톡 전용)"), new("!심몇개 (메시지)", ["!심몇개"], "답변 개수 확인"), new("!심랭킹 [개수]", ["!심랭킹"], "답변이 많은 메시지 TOP (최대 50개)")]),
        new("📊 통계", ["통계"], [new("!랭킹 [인원수]", ["!랭킹"], "채팅 랭킹 TOP (최대 50명)"), new("!내랭킹", ["!내랭킹"], "내 순위 확인"), new("!랭크 [개수]", ["!랭크"], "많이 올라온 채팅 TOP (최대 50개)"), new("!단어랭크 [개수]", ["!단어랭크"], "많이 사용된 단어 TOP (최대 50개)"), new("!정보", ["!정보"], "방 정보 및 통계"), new("!시간통계", ["!시간통계"], "시간대별 채팅 통계"), new("!내시간통계", ["!내시간통계"], "내 시간대별 채팅 통계"), new("!요일통계", ["!요일통계"], "요일별 채팅 통계"), new("!내요일통계", ["!내요일통계"], "내 요일별 채팅 통계"), new("!월별통계", ["!월별통계"], "월별 채팅 통계"), new("!내월별통계", ["!내월별통계"], "내 월별 채팅 통계")]),
        new("📈 주식", ["주식", "증시", "증권"], [new("!주식 [종목명/코드/티커]", ["!주식"], "현재가, 등락률, 장 상태 요약"), new("!주식상세 [종목명/코드/티커]", ["!주식상세"], "주요 지표, 기업 개요, 컨센서스 조회"), new("!주식차트 [종목명/코드/티커]", ["!주식차트"], "최근 일봉 가격/거래량 조회"), new("!주식뉴스 [종목명/코드/티커]", ["!주식뉴스"], "종목 뉴스 조회 (생략 시 증권 주요 뉴스)"), new("!증시 [국내/미국/나스닥/뉴욕/아멕스] [인기/시총/지수]", ["!증시"], "시장 지수와 종목 순위 조회"), new("!환율 [금액+출발통화] [도착통화]", ["!환율"], "Daum 환율 기준으로 통화를 변환합니다. 금액은 출발 통화에 붙여 쓰고, 도착 통화는 공백 뒤에 입력합니다. 금액을 생략하면 해당 통화의 기본 단위로 계산하고, 도착 통화를 생략하면 원화로 계산합니다. 숫자는 100달러처럼 써도 되고 1억4천만5백만달러처럼 한국어 단위를 섞어도 됩니다. 예: !환율, !환율 달러 엔, !환율 100엔 달러, !환율 1억4천만5백만달러")]),
        new("👮 관리자", ["관리", "관리자", "관리[자]"], [new("!관리추가", ["!관리추가"], "관리자 승인 요청"), new("!관리추가 (코드)", ["!관리추가"], "관리자 승인 (최고 관리자 전용)"), new("!관리제거 (SenderHash)", ["!관리제거"], "관리자 제거 (최고 관리자 전용)"), new("!관리목록", ["!관리목록"], "등록된 관리자 목록 조회 (관리자 전용)"), new("!멀티메시지", ["!멀티메시지"], "pending 메시지 일괄 전송 활성화 (최고 관리자 전용)"), new("!싱글메시지", ["!싱글메시지"], "pending 메시지 단일 전송 활성화 (최고 관리자 전용)"), new("!제한설정 (횟수)", ["!제한설정"], "방의 1일 요청 제한 설정 (관리자 전용)"), new("!제한해제", ["!제한해제"], "방의 요청 제한 해제 (관리자 전용)"), new("!랭크활성화", ["!랭크활성화"], "메시지 내용 랭킹 활성화 (관리자 전용)"), new("!랭크비활성화", ["!랭크비활성화"], "메시지 내용 랭킹 비활성화 (관리자 전용)"), new("!심삭제 (메시지)", ["!심삭제"], "심심이 답변 삭제 (관리자 전용)"), new("!반복설정", ["!반복설정"], "반복 메시지 설정 (관리자 전용)"), new("!반복해제 (번호/전체)", ["!반복해제"], "반복 메시지 해제 (관리자 전용)"), new("!반복목록", ["!반복목록"], "반복 메시지 목록 조회 (관리자 전용)"), new("!용아맥설정 → !아이맥스설정", ["!용아맥설정", "!아이맥스설정"], "IMAX 알림 등록 (관리자 전용)"), new("!아이맥스해제 (번호/전체)", ["!아이맥스해제"], "IMAX 알림 해제 (관리자 전용)"), new("!아이맥스목록", ["!아이맥스목록"], "IMAX 알림 목록 조회 (관리자 전용)"), new("!방백업", ["!방백업"], "방 데이터 백업 코드 생성 (관리자 전용)"), new("!방복원 (코드)", ["!방복원"], "백업 데이터로 방 복원 (관리자 전용)")]),
        new("⚾ 야구", ["야구"], [new("!야구팀등록 [팀명]", ["!야구팀등록"], "응원팀 등록 (KBO 팀, 팀명 생략 시 현재 등록팀 확인)"), new("!야구팀등록해제", ["!야구팀등록해제"], "응원팀 등록 해제"), new("!야구팀순위 [팀명, 생략 시 전체 팀 성적 요약]", ["!야구팀순위"], "KBO 팀 순위 조회"), new("!야구타자순위", ["!야구타자순위"], "타율 TOP5, 홈런 TOP5 조회"), new("!야구투수순위", ["!야구투수순위"], "평균자책점 TOP5, 승리 TOP5 조회"), new("!야구관중순위", ["!야구관중순위"], "구단별 관중 수 순위 조회"), new("!야구뉴스", ["!야구뉴스"], "오늘 기준 KBO 뉴스 조회 (다음날 오전6시까지 오늘)"), new("!오늘야구 [팀명]", ["!오늘야구", "!야구"], "오늘 야구 경기 조회 (KBO·대표팀, 팀명 입력 시 상세 정보)"), new("!내일야구 [팀명]", ["!내일야구"], "내일 야구 경기 조회 (KBO·대표팀, 팀명 입력 시 경기 전 상세 정보)"), new("!어제야구 [팀명]", ["!어제야구"], "어제 야구 경기 조회 (KBO·대표팀, 팀명 입력 시 상세 정보)"), new("!야구구독 [팀명]", ["!야구구독"], "경기 라인업/이벤트/점수 알림 구독 (KBO 팀 전용)"), new("!야구구독해제 [팀명]", ["!야구구독해제"], "야구 경기 알림 구독 해제")]),
        new("ℹ️ 기타", ["기타"], [new("!날씨 [지역]", ["!날씨"], "현재 날씨 확인 (기본: 이전 도시 또는 서울)"), new("!내일날씨 [지역]", ["!내일날씨"], "내일 날씨 확인 (기본: 이전 도시 또는 서울)"), new("@Here", ["@Here", "Here"], "최근 10일간 활동한 사용자 호출 (일반 사용자 24시간 1회)"), new("@everyone", ["@everyone", "everyone"], "방의 알려진 전체 사용자 호출 (일반 사용자 24시간 1회)"), new("!핫딜", ["!핫딜"], "랜덤 핫딜 상품 추천"), new("!영화목록 [검색어]", ["!영화목록"], "CGV 영화 목록 조회 (영화관 선택)"), new("!아이맥스조회 [영화이름]", ["!아이맥스조회"], "IMAX 시간표 조회 (영화관 선택)"), new("!검색 [검색어]", ["!검색"], "웹 검색 결과 (최대 10개)"), new("!뉴스 [검색어]", ["!뉴스"], "최신 뉴스 검색 결과 (최대 10개)"), new("!한강", ["!한강"], "한강 수질 정보 조회"), new("!단위 [수치+단위] [목적지 단위]", ["!단위"], "단위 변환 (길이·넓이·부피·무게·온도·속도·토크·압력·데이터 용량)"), new("!도움 / !도움말 / !help", ["!도움", "!도움말", "!help"], "이 메시지")])
    ];
    ...
    public Task<ServerResponse> HandleAsync(KakaoMessageData data)
    {
        try
        {
            ...
            var message = string.IsNullOrWhiteSpace(parameter) ? BuildFullHelpMessage() : BuildParameterizedHelpMessage(parameter);

            logger.LogInformation("[HELP] Showing help message to {Sender} in room {RoomId}", data.SenderName, data.RoomId);
    ...
    private static bool TryGetHelpParameter(string content, out string parameter)
    {
        ...
        foreach (var helpCommand in s_helpCommands)
        {
            ...
            if (trimmedContent.Length <= helpCommand.Length || !trimmedContent.StartsWith(helpCommand, StringComparison.OrdinalIgnoreCase) || !char.IsWhiteSpace(trimmedContent[helpCommand.Length]))
            {
                continue;
            }
    ...
    private static HelpCategory? FindCategory(string parameter)
    {
        ...
        return s_categories.FirstOrDefault(category => category.Keywords.Any(keyword => keyword.Equals(trimmedParameter, StringComparison.OrdinalIgnoreCase)));
    }
    ...
    private static bool IsCommandMatch(string normalizedParameter, string command, bool useExactMatch)
    {
        ...
        return useExactMatch ? normalizedCommand.Equals(normalizedParameter, StringComparison.OrdinalIgnoreCase) : normalizedCommand.Contains(normalizedParameter, StringComparison.OrdinalIgnoreCase);
    }
    ...
    private static string BuildNotFoundMessage(string parameter)
    {
        return $"'{parameter}'에 해당하는 도움말을 찾을 수 없습니다.\n" +
               "카테고리: 게임 / 재미 / AI / 심심이 / 통계 / 주식 / 관리자 / 야구 / 기타";
    }
}
