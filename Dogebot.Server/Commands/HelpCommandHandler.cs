...
public class HelpCommandHandler(ILogger<HelpCommandHandler> logger) : ICommandHandler
{
    ...
    [
        ...
        new("⚾ 야구", ["야구"], [new("!야구팀등록 [팀명]", ["!야구팀등록"], "응원팀 등록 (팀명 생략 시 현재 등록팀 확인)"), new("!야구팀등록해제", ["!야구팀등록해제"], "응원팀 등록 해제"), new("!야구팀순위 [팀명, 생략 시 전체 팀 성적 요약]", ["!야구팀순위"], "KBO 팀 순위 조회"), new("!야구타자순위", ["!야구타자순위"], "타율 TOP5, 홈런 TOP5 조회"), new("!야구투수순위", ["!야구투수순위"], "평균자책점 TOP5, 승리 TOP5 조회"), new("!야구관중순위", ["!야구관중순위"], "구단별 관중 수 순위 조회"), new("!야구뉴스", ["!야구뉴스"], "오늘 기준 KBO 뉴스 조회 (다음날 오전6시까지 오늘)"), new("!오늘야구 [팀명]", ["!오늘야구", "!야구"], "오늘 KBO 경기 조회 (팀명 입력 시 상세 정보)"), new("!내일야구 [팀명]", ["!내일야구"], "내일 KBO 경기 조회 (팀명 입력 시 경기 전 상세 정보)"), new("!어제야구 [팀명]", ["!어제야구"], "어제 KBO 경기 조회 (팀명 입력 시 상세 정보)"), new("!야구구독 [팀명]", ["!야구구독"], "경기 라인업/이벤트/점수 알림 구독"), new("!야구구독해제 [팀명]", ["!야구구독해제"], "야구 경기 알림 구독 해제")]),
