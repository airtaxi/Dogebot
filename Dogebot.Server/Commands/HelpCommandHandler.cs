...
public class HelpCommandHandler(ILogger<HelpCommandHandler> logger) : ICommandHandler
{
    ...
    [
        ...
        new("📊 통계", ["통계"], [
            new("!랭킹", ["!랭킹"], "랭킹 조회 방법 안내"),
            new("!조회 (roomId)", ["!조회"], "채팅 랭킹 TOP 10"),
            new("!내랭킹", ["!내랭킹"], "내 순위 확인"),
        ...
        new("📈 주식", ["주식", "증시", "증권"], [
            new("!주식 [종목명/코드/티커]", ["!주식"], "현재가, 등락률, 장 상태 요약"),
            new("!주식상세 [종목명/코드/티커]", ["!주식상세"], "주요 지표, 기업 개요, 컨센서스 조회"),
            new("!주식차트 [종목명/코드/티커]", ["!주식차트"], "최근 일봉 가격/거래량 조회"),
            new("!주식뉴스 [종목명/코드/티커]", ["!주식뉴스"], "종목 뉴스 조회 (생략 시 증권 주요 뉴스)"),
            new("!증시 [국내/미국/나스닥/뉴욕/아멕스] [인기/시총/지수]", ["!증시"], "시장 지수와 종목 순위 조회")
        ]),
        new("👮 관리자", ["관리", "관리자", "관리[자]"], [
