...
public class ImaxNotificationService : IImaxNotificationService
{
    ...
    private async Task<List<ServerResponseItem>> FindAndDeletePendingMessagesAsync(FilterDefinition<ImaxNotification> filter)
    {
        ...
        {
            var notification = await _imaxNotifications.FindOneAndDeleteAsync(filter);
            if (notification is null) return responseItems;
